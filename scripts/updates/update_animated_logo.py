...
export function AnimatedLogo({ className = '', text = 'SpaceQuiz', isHovered: externalHover }: AnimatedLogoProps) {
  ...
  return (
    ...
    >
      ...
      <div 
        ref={iconRef} 
        className="relative flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 z-10 transition-colors duration-500"
      >
        {/* Premium multi-layered glow */}
        <div className="absolute inset-0 bg-gradient-to-tr from-cyan-400 via-indigo-500 to-purple-500 rounded-xl sm:rounded-2xl opacity-40 blur-xl group-hover:opacity-80 group-hover:blur-2xl transition-all duration-700" />
        
        {/* Solid background shape */}
        <div className="absolute inset-0 bg-slate-900 dark:bg-slate-950 rounded-xl sm:rounded-2xl border border-white/20 dark:border-slate-700/50 shadow-[inset_0_1px_1px_rgba(255,255,255,0.2)] overflow-hidden">
        ...
        <svg viewBox="0 0 100 100" className="w-5 h-5 sm:w-7 sm:h-7 relative z-10 drop-shadow-[0_0_8px_rgba(255,255,255,0.4)]">
          <defs>
      ...
      <span 
        ref={textRef}
        className="relative font-display font-black text-2xl sm:text-3xl tracking-tight z-10 pt-1"
      >
...
with open('src/components/AnimatedLogo.tsx', 'w') as f:
    f.write(content)
print("Updated AnimatedLogo.tsx")
