...
export default function App() {
  ...
  const setActiveTab = React.useCallback((tab: string) => {
    ...
    if (isProfile) {
      ...
      window.history.pushState(null, '', `/#/profile/${profileId}?${nextParams.toString()}`);
    } else {
      nextParams.delete('profileId');
      window.history.pushState(null, '', `/#/dashboard/${tab}?${nextParams.toString()}`);
    }
  }, [userId]);
  ...
  const setActiveQuizId = React.useCallback((quizId: string | null) => {
    ...
    if (quizId) {
      ...
      window.history.pushState(null, '', `/#/quiz/${quizId}?${nextParams.toString()}`);
    } else {
  ...
  React.useEffect(() => {
    if (userId && !userId.startsWith('user-') && isStatsLoaded) {
      ...
      // Sync user premium state dynamically to Express backup database
      try {
        fetch(getApiUrl(`/api/profiles/${userId}/premium`), {
          method: 'POST',
        ...
        }).catch((err) => console.warn('Deferred premium REST sync:', err));
      } catch (_) {}
    }
  ...
  React.useEffect(() => {
    ...
    // Subscribe to Firestore /notifications collection in real-time
    if (!db) return;
    if (localStorage.getItem('firebase_quota_exceeded') === 'true') {
      return;
    }
    ...
    const qSnap = query(
      notifCol,
      where('createdAt', '>=', sessionBootTime),
      orderBy('createdAt', 'desc'),
      limit(5)
    );

    const unsubscribeFirestore = onSnapshot(qSnap, (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === 'added') {
          const data = change.doc.data();
          // Skip own messages
          if (data.senderName && data.senderName === userName) return;

          // Dispatch beautiful push toast
          ...
          // Play a delightful synth effect
          try {
      ...
      console.warn('Real-time notification listener sync deferred:', error);
    });
  ...
  React.useEffect(() => {
    ...
    const sessionBootTime = new Date().toISOString();
    const chatsCol = collection(db, 'direct_messages');
    const q = query(
      chatsCol,
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docChanges().forEach((change) => {
        if (change.type === 'added') {
          ...
          // Only trigger for documents created in the database during this active session
          const isNew = msg.createdAt && new Date(msg.createdAt).getTime() > new Date(sessionBootTime).getTime();
          if (isNew) {
            const senderName = msg.senderName || (lang === 'ar' ? 'مستخدم' : 'Scholar');
            const text = msg.text || '';
            ...
            // Also dispatch system screen toast
            pushNotificationsManager.trigger({
      ...
      console.warn('Silent unread chats real-time counter deferred:', error);
    });
  ...
  React.useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user: any) => {
      if (user && !user.isAnonymous) {
        ...
        localStorage.setItem('quiz_userId', user.uid);
        ...
        if (pendingQuizId && pendingQuizId !== 'login-trigger') {
          ...
          window.history.pushState(null, '', `/#/quiz/${pendingQuizId}`);
        ...
        if (pendingTab) {
          ...
          if (pendingTab === 'profile') {
            setUrlProfileId(user.uid);
            window.history.pushState(null, '', `/#/profile/${user.uid}`);
          } else {
        ...
        try {
          await saveUserProfile(user.uid, finalName, user.photoURL || undefined, user.email || undefined, undefined, undefined, undefined, undefined, undefined, true);
        } catch (e) {
        ...
        try {
          ...
          try {
            const cachedKey = `user_profile_fallback_${user.uid}`;
            const existingDataStr = localStorage.getItem(cachedKey);
            if (existingDataStr) {
        ...
        if (!storedUserId || storedUserId.startsWith('user-')) {
          if (!storedUserId) {
            storedUserId = 'user-' + Math.random().toString(36).substring(2, 11);
            localStorage.setItem('quiz_userId', storedUserId);
          }
          setUserId(storedUserId);
          setUserName(localStorage.getItem('quiz_userName') || 'طالب زائر');
          setUserEmail(null);
        } else {
          // Reset session safely
          const guestId = 'user-' + Math.random().toString(36).substring(2, 11);
          localStorage.setItem('quiz_userId', guestId);
          setUserId(guestId);
  ...
  React.useEffect(() => {
    ...
    // 2. Direct Shared Link check for Guest auth redirection
    const path = window.location.pathname;
    const hash = window.location.hash;
  ...
  const handleSetTab = (tab: string, bypassAuth = false, overrideProfileId: string | null = null) => {
    ...
    // Guest Auth Guard for Profile and Create studio tabs (except if viewing another public creator profile)
    const isGuest = !authContext.isAuthenticated && (!auth.currentUser || auth.currentUser.isAnonymous || !auth.currentUser.email);
    ...
    if (tab === 'profile') {
      ...
      window.history.pushState(null, '', `/#/profile/${targetProfileId}?${currentParams.toString()}`);
    } else {
      currentParams.delete('profileId');
      window.history.pushState(null, '', `/#/dashboard/${tab}?${currentParams.toString()}`);
    }
  };
  ...
  return (
      ...
      <div className="flex-1 flex flex-col min-w-0 transition-colors duration-300 relative overflow-x-hidden">                
        ...
        <main ref={mainContainerRef} className="flex-1 w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-8 relative z-10 pt-4">
          ...
          {/* Prompt to create a custom ID if logged in but customId is not set */}
          {userId && !userId.startsWith('user-') && !userCustomId && (
            <div 
              
              
              className="mb-6 p-4 rounded-3xl bg-gradient-to-r from-primary/15 via-violet-500/10 to-transparent border border-primary/20 flex flex-col md:flex-row items-center justify-between gap-4 relative overflow-hidden backdrop-blur-md shadow-sm"
            >
              <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-full blur-2xl pointer-events-none" />
              <div className="flex items-center gap-3">
                <div className="p-3 rounded-2xl bg-primary/25 text-primary shrink-0">
                  <Sparkles className="w-5 h-5 animate-pulse text-primary" />
                </div>
                <div className="space-y-1 text-right md:text-left">
                  <h4 className="font-display font-black text-sm text-slate-800 dark:text-slate-100 flex items-center gap-1.5 justify-center md:justify-start">
                    {lang === 'ar' ? '✨ خصّص معرّف حسابك الفريد (Custom ID)!' : '✨ Set Your Custom User ID!'}
                  </h4>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {lang === 'ar' 
                      ? 'لم تقم بإنشاء معرّف حساب مخصص حتى الآن. أنشئ معرّفك المميّز لتتمكن من تفعيل باقات بريميوم والتحقق من حسابك بسهولة!' 
                      : 'You haven\'t set a custom ID yet. Create your custom ID to allow easy activation of premium plans and account verification!'}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="px-4 py-2 bg-primary hover:bg-primary-hover text-white rounded-xl text-xs font-black transition-all shadow-md shadow-primary/10 hover:-translate-y-0.5 active:translate-y-0 cursor-pointer"
                >
                  {lang === 'ar' ? 'إنشاء معرّف الحساب (ID) الآن 🆔' : 'Create Custom ID Now 🆔'}
                </button>
              </div>
            </div>
          )}
      ...
      {/* Beautiful Forced/Manual Welcoming Login Screen Overlay */}
          ...
          <WelcomeAuthOverlay
            ...
            onClose={() => {
              ...
              if (isAuthQuiz && authRedirectQuizId) {
                ...
                window.history.pushState(null, '', `/#/quiz/${authRedirectQuizId}`);
              }
              authRedirectQuizIdRef.current = null;
              setAuthRedirectQuizId(null);
      ...
      <AuthModal 
        ...
        onSuccess={(user, token) => {
          ...
          if (user.photoURL) {
            localStorage.setItem('quiz_userPhoto', user.photoURL);
          ...
          // Update Custom AuthContext session to prevent route ejection or guest redirects
          try {
            authContext.supabase.auth.setSession({
              access_token: token,
              user: {
                id: user.uid,
                uid: user.uid,
            ...
            console.warn('Sync auth context failed:', e);
          }
      ...
      {/* Pulsing Assistant Toggle Button or Support Button */}
        ...
        {!showAssistant && !activeQuizId && (
          <div className={`fixed bottom-24 sm:bottom-20 ${lang === 'ar' ? 'left-6' : 'right-6'} z-40`}>
            {isUserPremium ? (
              <>
