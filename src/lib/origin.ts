...
/**
 * Initializes and fetches the real server origin.
 */
export async function initAppOrigin(): Promise<string> {
  try {
    const res = await fetch(getApiUrl('/api/share-origin'));
    if (res.ok) {
      const data = await res.json();
      if (data.origin && !data.origin.includes('aistudio.google.com')) {
        localStorage.setItem('quiz_app_origin', data.origin);
        return data.origin;
      }
    }
  } catch (err) {
    console.error('Failed to fetch real share origin from backend:', err);
  }
  return getAppOrigin();
}
