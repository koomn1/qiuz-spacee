...
export default function Classrooms({
  ...
  useEffect(() => {
    const fetchClassroomsData = async () => {
      try {
        const r1 = await fetch(getApiUrl('/api/classrooms'));
        const d1 = await r1.json();
        if (Array.isArray(d1) && d1.length > 0) setClassrooms(d1);
        
        const r2 = await fetch(getApiUrl('/api/classrooms/students'));
        const d2 = await r2.json();
        if (Array.isArray(d2) && d2.length > 0) setClassroomStudents(d2);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const loadMessages = () => {
      fetch(getApiUrl(`/api/classrooms/${activeClassroomView.id}/messages`))
        .then(res => res.json())
        .then(data => {
          if (!isMounted) return;
          if (Array.isArray(data)) {
            setActiveClassroomMessages(prev => {
              if (!initialLoad && data.length > prev.length) {
        ...
        .catch(err => console.error('Error loading classroom messages:', err));
    };
  ...
  const handleJoinByCode = async (codeToJoin?: string) => {
    ...
    setErrorText(null);
    try {
      const response = await fetch(getApiUrl('/api/classrooms/join'), {
        method: 'POST',
      ...
      const freshClassroomsRes = await fetch(getApiUrl('/api/classrooms'));
      const freshClassrooms = await freshClassroomsRes.json();
      if (Array.isArray(freshClassrooms)) {
        setClassrooms(freshClassrooms);
        const joinedClass = freshClassrooms.find(c => c.code.toUpperCase() === targetCode);
        if (joinedClass) {
          playChimeSound('correct');
  ...
  const handleSendChatMessage = async () => {
    ...
    try {
      ...
      const response = await fetch(getApiUrl(`/api/classrooms/${activeClassroomView.id}/messages`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleTriggerAiQuiz = async () => {
    ...
    try {
      const response = await fetch(getApiUrl('/api/ai/quiz-draft'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          topic: aiQuizTopic,
          classroomId: activeClassroomView.id,
          language: lang
        })
      });
      if (response.ok) {
        ...
        setAiQuizTopic('');
        triggerToast(
          isAr ? '🤖 تم توليد الاختبار بالذكاء الاصطناعي!' : '🤖 AI Quiz Draft Generated!',
          isAr ? 'الكويز جاهز في صفحة "إنشاء اختبار" للمراجعة والنشر.' : 'Check the Quiz Creator to finalize and share!',
          'quiz'
        );
      }
    } catch (err) {
  ...
  const handleUpdatePermissions = async (allowMessages: boolean, allowMedia: boolean) => {
    if (!activeClassroomView) return;
    try {
      const response = await fetch(getApiUrl(`/api/classrooms/${activeClassroomView.id}/permissions`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ allowStudentMessages: allowMessages, allowStudentMedia: allowMedia })
      });
      if (response.ok) {
