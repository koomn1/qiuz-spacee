...
export default function DrivePicker({ isOpen, onClose, onFileSelected, lang }: DrivePickerProps) {
  ...
  // Auto-connect with Firebase Auth if possible, or trigger login popup
  const handleConnect = async () => {
    ...
    try {
      ...
      if (!token) {
        throw new Error(isAr ? 'لم نتمكن من الحصول على صلاحية الوصول من Google.' : 'Could not retrieve Google access token.');
      }
      ...
      if (userId) {
        localStorage.setItem(`gdrive_linked_${userId}`, 'true');
        localStorage.setItem(`gdrive_account_${userId}`, email);
      }
    } catch (err: any) {
