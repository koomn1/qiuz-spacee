...
export default function QuizResolver({
  ...
  const handleFetchAiFlashcardExplanation = async (qId: string, q: Question) => {
    ...
    try {
      ...
      const res = await fetchWithAuth(getApiUrl('/api/explain-question'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          questionText: q.text,
          userAnswer: isAr ? 'أرغب بشرح نظري مبسط' : 'I just want a simplified theory explanation.',
          correctAnswer,
          options: q.options || [],
          lang: lang
        })
      ...
      if (data.explanation) {
        setAiFlashcardExplanations(prev => ({ ...prev, [qId]: data.explanation }));
      } else {
  ...
  const handleFinalSubmitAndExit = async () => {
    ...
    try {
      if (savedCompletionId) {
        // Update the existing completion with rating/feedback and update quiz analytics
        const res = await fetch(`/api/completions/${savedCompletionId}/rate`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        if (!res.ok) {
          throw new Error('Rating update failed on backend');
        }
      } else {
        // Fallback: create fresh completion
        const finalName = takerName.trim() || userName.trim() || 'طالب متميز';
        await submitQuizAttempt(quizId, {
          takerId: userId || 'anonymous',
          takerName: finalName,
      ...
      console.error(err);
      alert(lang === 'ar' ? 'فشل حفظ التقييم ومحاولة الخروج.' : 'Failed to save rating and exit.');
    } finally {
      setIsSubmittingReview(false);
    }
  };
