...
import { createPremiumRequest, getPremiumRequests } from '../lib/db';
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (coupon.expiryDate) {
          ...
          if (expDate < nowStr) {
            alert(isAr 
              ? `عذراً، كود الخصم منتهي الصلاحية! تاريخ الانتهاء كان: ${new Date(coupon.expiryDate).toLocaleDateString(isAr ? 'ar-EG' : 'en-US')}` 
              : `Sorry, this coupon is expired! Expiry was: ${new Date(coupon.expiryDate).toLocaleDateString()}`
            );
            setPromoDiscount(0);
            return;
          }
