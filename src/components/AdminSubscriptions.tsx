...
export default function AdminSubscriptions({
  ...
  const loadRealData = async () => {
    try {
      const { getAllProfiles, getPremiumRequests } = await import("../lib/db");
      ...
      const mappedUsers = profiles.map((p) => ({
        ...
        planName: p.planName || "Free",
        planId: p.planId || undefined,
        badgeSymbol: p.isPremium ? "✔" : "",
        createdQuizzes: [],
        completions: [],
      }));
  ...
  const handleSaveCoupon = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(isAr ? "فشل حفظ الكود." : "Failed to save coupon.");
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const handleDeleteCoupon = async (id: string) => {
    if (
      ...
      return;
    try {
      const res = await fetchWithAuth(getApiUrl(`/api/coupons/${id}`), {
        method: "DELETE",
      });
      if (res.ok) {
        loadCoupons();
  ...
  const handleGenerateAiMessageForCoupon = async (
    ...
    try {
      const res = await fetchWithAuth(getApiUrl("/api/admin/generate-promo-msg"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        const data = await res.json();
        setAiPromoMsg(data.message);
      } else {
        alert(
          isAr
            ? "فشل توليد الرسالة بالذكاء الاصطناعي."
  ...
  const handleBroadcastMessage = async () => {
    ...
    try {
      const res = await fetchWithAuth(getApiUrl("/api/admin/broadcast-promo-msg"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          text: aiPromoMsg,
          senderId: "admin-cosmo",
          senderName: "المساعد كوزمو",
        }),
      });
      if (res.ok) {
        const data = await res.json();
        alert(
          isAr
            ? `تم بث الإعلان بنجاح ونشره بالمجتمع! وتم إرساله إلى عدد ${data.count || 0} مستخدم في الرسائل الخاصة.`
            : `Promotion broadcasted successfully! Posted to community feed and delivered as direct messages to ${data.count || 0} users.`,
        );
        setAiPromoMsg("");
      } else {
        alert(
          isAr
            ? "فشل إرسال البث لجميع الطلاب."
  ...
  const handleActivateSubscription = async () => {
    ...
    try {
      const res = await fetchWithAuth(getApiUrl("/api/admin/manual-activate"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (res.ok) {
        ...
        alert(
          isAr
            ? `فشل تحديث اشتراك العضو: ${errData.error || ""}`
            : `Failed to update subscription: ${errData.error || ""}`,
        );
      }
    } catch (err) {
      console.error(err);
  ...
  const approvePayment = async (payId: string) => {
    try {
      ...
      if (!payInfo) return;
      const res = await fetchWithAuth(getApiUrl(`/api/premium-requests/${payId}/status`), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          status: "approved",
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={async () => {
                    ...
                    // original mockup placeholder fallback:
                    alert(
                      isAr
                        ? `تم تفعيل حساب ${uidVal} لخطة ${planVal}`
                        : `Activated ${uidVal} to ${planVal}`,
                    );
                    ...
                    try {
                      ...
                      console.error(
                        "Failed to broadcast global activation notification",
                        e,
                      );
                    }
                  }}
