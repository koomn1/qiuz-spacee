    ...
    try {
      if (mode === 'register') {
        ...
        try {
          ...
          console.warn('Firebase registration failed, falling back to database auth:', firebaseErr.message);
          ...
          if (!response.ok) {
            ...
            throw new Error(data.error || 'فشلت عملية إنشاء الحساب المحلي.');
          }
        ...
        try {
          ...
          // Check if user has MFA/2FA enabled in Postgres backend
          const checkRes = await fetch('/api/auth/2fa/check', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email: cleanEmail, uid: userObj.uid })
          });

          if (checkRes.ok) {
            const checkData = await checkRes.json();
            if (checkData.enabled) {
              // Standard sign out of Firebase for now to hold the session until MFA OTP is entered
              await auth.signOut();
              setPending2FAUser({
                uid: userObj.uid,
                email: cleanEmail,
                name: userObj.displayName || 'طالب متميز',
                method: 'firebase',
                emailPassword: { email: cleanEmail, password: cleanPassword }
              });
              setStep('2fa');
              setLoading(false);
              return;
          ...
          console.warn('Firebase login failed, trying database fallback auth:', firebaseErr.message);
          ...
          const checkRes = await fetch('/api/auth/2fa/check', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email: cleanEmail, uid: data.user.uid })
          });
      ...
      let errMsg = err.message || 'حدث خطأ غير متوقع أثناء عملية الدخول';
      if (err.code === 'auth/email-already-in-use') {
        errMsg = 'هذا البريد الإلكتروني مستخدم بالفعل في حساب آخر.';
      } else if (err.code === 'auth/weak-password') {
        errMsg = 'كلمة المرور ضعيفة جداً! يرجى إدخال 6 خانات أو أكثر لحماية حسابك.';
      } else if (err.code === 'auth/invalid-email') {
        errMsg = 'صيغة البريد الإلكتروني المدخلة غير صحيحة.';
      } else if (err.code === 'auth/user-not-found' || err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential') {
        errMsg = 'عذراً! بيانات الدخول غير صحيحة. يرجى التحقق وإعادة المحاولة.';
      }
      setError(errMsg);
    } finally {
      setLoading(false);
  ...
  const handleVerify2FA = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/2fa/verify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ uid: pending2FAUser.uid, code: verificationCode })
      });

      if (res.ok) {
        // Complete authenticating session
        if (pending2FAUser.method === 'firebase') {
          // Relogin Firebase securely
          await signInWithEmailAndPassword(auth, pending2FAUser.emailPassword.email, pending2FAUser.emailPassword.password);
          onClose();
        } else {
          // Local fallback
          if (onSuccess) {
            onSuccess(pending2FAUser.localData.user, pending2FAUser.localData.token);
          }
        }
      } else {
      ...
      setError('فشل التحقق من المصادقة الثنائية.');
    } finally {
