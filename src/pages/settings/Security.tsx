...
export default function Security({ lang }: SecurityProps) {
  ...
  const fetchSessions = async () => {
    try {
      ...
      const res = await fetchWithAuth(getApiUrl('/api/security/sessions'));
      if (res.ok) {
  ...
  const fetch2FAStatus = async () => {
    try {
      ...
      const res = await fetchWithAuth(getApiUrl('/api/security/2fa/status'));
      if (res.ok) {
        const data = await res.json();
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user && user.email) {
        // Reauthenticate standard credentials before updating credentials safely
  ...
  const handleToggle2FA = async () => {
    ...
    if (is2FAEnabled) {
      ...
      try {
        const res = await fetchWithAuth(getApiUrl('/api/security/2fa/disable'), {
          method: 'POST'
        });
        if (res.ok) {
          ...
          setErrorMessage2FA(isAr ? 'فشل إلغاء تفعيل المصادقة الثنائية.' : 'Failed to disable Two-Factor Authentication.');
        }
      } catch (err) {
      ...
      try {
        const res = await fetchWithAuth(getApiUrl('/api/security/2fa/enroll'), {
          method: 'POST'
        });
        if (res.ok) {
          const data = await res.json();
          setEnrollmentSecret(data.secret);
          setEnrollmentQRCode(data.qrCode);
  ...
  const handleVerify2FA = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetchWithAuth(getApiUrl('/api/security/2fa/verify'), {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        setErrorMessage2FA(data.error || (isAr ? 'الرمز الذي أدخلته غير صحيح أو منتهي الصلاحية.' : 'The code you entered is incorrect or expired.'));
      }
    } catch (err) {
  ...
  const handleRevokeSession = async (sessionId: string) => {
    try {
      const res = await fetchWithAuth(getApiUrl('/api/security/sessions/revoke'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleLogOutAll = async () => {
    try {
      const res = await fetchWithAuth(getApiUrl('/api/security/sessions/revoke-all'), {
        method: 'POST'
      });
      if (res.ok) {
