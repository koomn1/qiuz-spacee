...
export default function CosmoChatbot({ lang, isPremium, planName, onUpgradeClick, onOpenAuthModal }: CosmoChatbotProps) {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetchWithAuth(getApiUrl('/api/cosmobot-chat'), {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          errorText = isAr 
            ? "🔒 عذراً! يجب تسجيل الدخول أولاً للتحدث مع كوزمو بوت واستخدام ميزات الذكاء الاصطناعي."
            : "🔒 Sorry! You must be logged in first to chat with Cosmo Bot and use AI features.";
        } else {
          try {
