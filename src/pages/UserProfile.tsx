...
export default function UserProfile({
  ...
  const fetchFollowStats = async () => {
    ...
    try {
      const res = await fetch(
        `/api/follows/${profileId}?viewerId=${currentUserId}`,
      );
      if (res.ok) {
  ...
  const handleCheckCustomId = async (id: string) => {
    ...
    try {
      const res = await fetch(
        `/api/profiles/check-custom-id/${clean}?userId=${profileId}`,
      );
      if (res.ok) {
        const data = await res.json();
        if (!data.available) {
  ...
  const handleSaveProfile = async () => {
    if (editCustomId.trim()) {
      ...
      try {
        const res = await fetch(
          `/api/profiles/check-custom-id/${cleanCustomId}?userId=${profileId}`,
        );
        if (res.ok) {
          const data = await res.json();
          if (!data.available) {
    ...
    try {
      if (auth.currentUser) {
        try {
          // Firebase Auth has a 2048-char limit on photoURL. If it's a base64 string, it will fail.
          // We only pass photoURL to auth.currentUser if it's not a data URI and its length is <= 2048.
          const safePhotoURL =
            editPhotoURL &&
            !editPhotoURL.startsWith("data:") &&
            editPhotoURL.length <= 2048
              ? editPhotoURL
              : auth.currentUser.photoURL &&
                  !auth.currentUser.photoURL.startsWith("data:") &&
                  auth.currentUser.photoURL.length <= 2048
                ? auth.currentUser.photoURL
                : "";
          ...
          console.warn(
            "Could not update Firebase Auth profile attributes (photo URL too long or invalid), but continuing with DB save:",
            authError,
          );
          // We can still try to at least update the displayName
          try {
            await updateProfile(auth.currentUser, {
              displayName: editName,
            });
      ...
      if (code === "QUIZ50" || code === "SUPER50" || code === "GEMINI55") {
        ...
        if (res.ok) {
          ...
          if (couponObj.isActive) {
            ...
            if (
              couponObj.maxUses &&
              couponObj.usedCount !== undefined &&
              couponObj.usedCount >= couponObj.maxUses
            ) {
              alert(
                isAr
