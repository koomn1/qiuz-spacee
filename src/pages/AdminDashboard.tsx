...
export default function AdminDashboard({ quizzes, lang, onViewProfile, currentUserId, currentUserEmail }: AdminDashboardProps) {
  ...
  useEffect(() => {
    ...
    fetch(getApiUrl('/api/classrooms'))
      .then(res => res.json())
      .then(data => { if (Array.isArray(data)) setAdminClassrooms(data); })
      .catch(err => console.error('Error fetching admin classrooms:', err));

    fetch(getApiUrl('/api/classrooms/students'))
      .then(res => res.json())
      .then(data => { if (Array.isArray(data)) setAdminStudents(data); })
      .catch(err => console.error('Error fetching admin students:', err));
  }, []);

  // Fetch classroom messages if active
  useEffect(() => {
    if (activeAdminClassroom) {
      fetch(getApiUrl(`/api/classrooms/${activeAdminClassroom.id}/messages`))
        .then(res => res.json())
        .then(data => { if (Array.isArray(data)) setAdminMessages(data); })
        .catch(err => console.error('Error loading classroom messages for admin:', err));
    }
                           ...
                           <tr key={u.uid || u.id || idx} className="border-b border-slate-200/50 dark:border-slate-700/50">
                             ...
                             >
                               <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden shrink-0">
                                 {u.photoURL ? <img src={u.photoURL} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center font-bold text-xs">{u.name?.charAt(0) || 'U'}</div>}
                               </div>
                               {u.name || 'User'}
                             </td>
                             <td className="px-6 py-4 text-slate-500">{u.email || u.uid}</td>
                             <td className="px-6 py-4">
                               <span className={`px-2 py-1 ${u.isPremium ? 'bg-amber-500/10 text-amber-500' : 'bg-emerald-500/10 text-emerald-500'} rounded-md text-xs font-bold`}>
                                 {u.isPremium ? 'Premium' : 'Active'}
                               </span>
                             </td>
