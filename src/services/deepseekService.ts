...
تأكد من أن الاستجابة تطابق تماماً الهيكل المطلوب:
{
  ...
  const response = await fetchWithAuth(getApiUrl('/api/generate/deepseek'), {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    let errorMsg = 'DeepSeek Quiz Generation Service failed.';
    try {
  ...
  try {
    ...
    console.error('Failed to parse DeepSeek response text:', cleanedText);
