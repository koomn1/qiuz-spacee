...
تأكد من أن الاستجابة تطابق تماماً الهيكل المطلوب:
{
  ...
  const response = await fetchWithAuth(getApiUrl('/api/generate/groq'), {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    let errorMsg = 'Groq Quiz Generation Service failed.';
    try {
  ...
  try {
    ...
    console.error('Failed to parse Groq response text:', cleanedText);
