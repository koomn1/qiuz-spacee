...
تأكد من أن الاستجابة تطابق تماماً الهيكل المطلوب:
{
  ...
  const response = await fetchWithAuth(getApiUrl('/api/generate/openai'), {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    let errorMsg = 'OpenAI Quiz Generation Service failed.';
    try {
  ...
  try {
    ...
    console.error('Failed to parse OpenAI response text:', cleanedText);
